...
async function generateSpeakingTurn(apiKey, level, topic, history, recentFocus) {
  ...
  const prompt = isOpening
    ? `אתה שותף שיחה סבלני וממריץ לתרגול אנגלית מדוברת, עבור לומד ישראלי ברמת "${levelHe}"${topic ? ` בנושא "${topic}"` : ""}.${focusNote}
פתח בשאלה קצרה, טבעית וידידותית באנגלית שתתחיל שיחה קצרה בנושא הזה.
החזר אובייקט JSON בפורמט הבא בדיוק:
{"reply": "המשפט/שאלה הפותחת שלך באנגלית, קצרה וטבעית", "replyHe": "תרגום לעברית של reply", "correction": null, "feedbackHe": null, "mistakeTag": null, "hint": ["2-3 מילים או ביטויים קצרים באנגלית שיכולים לעזור לתלמיד לענות"]}
החזר אך ורק JSON תקין, בלי שום טקסט נוסף.`
    : `אתה שותף שיחה סבלני וממריץ לתרגול אנגלית מדוברת, עבור לומד ישראלי ברמת "${levelHe}"${topic ? ` בנושא "${topic}"` : ""}.${focusNote}
...
function Home({ state, setLevel, dueCount, newCount, onGo }) {
  ...
  const recentFocus = summarizeFocusAreas(state.speakingMemory?.mistakeTags || []);
  return (
    <div>
      ...
      {recentFocus && (
        <div className="ela-note" style={{ background: "var(--sage-light)", color: "var(--sage)", borderRadius: 12, padding: "10px 12px", marginBottom: 12 }}>
...
function SpeakingPractice({ level, apiKey, recentFocus, onMistakeTag, onBack, onSessionEnd }) {
  ...
  async function startTopic(t) {
    ...
    setError(null);
    try {
      const turn = await generateSpeakingTurn(apiKey, effectiveLevel, t, [], recentFocus);
  ...
  async function sendAnswer(text) {
    ...
    setTranscript("");
    try {
      const turn = await generateSpeakingTurn(apiKey, effectiveLevel, topic, nextHistory, recentFocus);
...
export default function App() {
  ...
  function recordMistakeTag(tag) {
    const mistakeTags = [...(state.speakingMemory?.mistakeTags || []), tag].slice(-30);
    persist({ ...state, speakingMemory: { ...state.speakingMemory, mistakeTags } });
  }
